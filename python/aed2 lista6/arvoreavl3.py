...
def ver_balanco(T):
    ...
    if T['fb']>0:
        if T['dir']['fb']<0:#rota dupla a esq// esq para dir no filho da dir e dir para esq na raiz
            T['dir'] = rotacaoLL(T['dir'])
            T = rotacaoRR(T)
        else:
            T = rotacaoRR(T)
    else:
        if T['esq']['fb']>0:
            T['esq'] = rotacaoRR(T['esq'])
            T = rotacaoLL(T)
        else:
            T = rotacaoLL(T)
